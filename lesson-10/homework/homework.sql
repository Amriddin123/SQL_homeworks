...
--1- task
select Name, Salary, DepartmentName from Employees as emp 
join 
 Departments as dep on emp.DepartmentID = dep.DepartmentID
 where salary  > 50000

-- 2- task
	
	select FirstName, LastName , OrderDate from Customers as cust
	join 
	Orders as ods on cust.CustomerID = ods.CustomerID
	where  year(OrderDate)  = 2023 	

	---3-task

	select Name, DepartmentName from Employees as emp
 	left join  Departments as dep on emp.DepartmentID = dep.DepartmentID
	--4-task 
	select * from Products
	select * from Suppliers
	
	select SupplierName, ProductName from Suppliers as sup
	left join Products as pro on sup.SupplierID = pro.SupplierID
...
select emp.name , emp2.ManagerName from Employees as emp 
join Employees as emp2
 on emp.ManagerID = emp2.ManagerID

 --7task 
 select * from Students
 select * from Courses
 select name ,CourseName from Enrollments as enro
 join Courses as cour on enro.CourseID = cour.CourseID
 join Students as st on enro.StudentID = st.StudentID
 ...
 select FirstName , LastName, Quantity from Customers as cust
 join Orders as od on cust.CustomerID = od.CustomerID
 where Quantity >= 3
