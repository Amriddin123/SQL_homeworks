...
--6-task
select max(Price) from Products as max_product

--7-task

select StockQuantity ,
 case 
 when StockQuantity < 30 then 'kam'
 else 'yetarli' end as stockStatus
 from Products

 --8-task
 select Country, count(*) as costumercount
 from Customers
 group by Country

--9-task
select max(Quantity) from Orders as max_Quantity
select min(Quantity) from Orders as min_Quantity
