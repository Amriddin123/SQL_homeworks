...
--1-task
select 
 left(name,CHARINDEX(',', Name )-1) as ism,
 right(name,len(name) - CHARINDEX(',', Name )) as familya
...
select *, LEN(Vals) - LEN(REPLACE(Vals,'.',''))
 from testDots 
...
CREATE TABLE CountSpacess
(
texts VARCHAR(100)
)

 
INSERT INTO CountSpacess VALUES
('P Q R S '),
(' L M N O 0 0     '),
('I  am here only '),
...
select * from CountSpacess

select texts,
len(texts) - len(REPLACE(texts,' ', '')) from CountSpacess
--6-task
...
select emp1.Id, emp1.Name,emp1.Salary,emp2.Name ,emp2.Salary,emp2.ManagerId from Employee as emp1
...
select EMPLOYEE_ID, FIRST_NAME, LAST_NAME, HIRE_DATE, DATEDIFF(YEAR,HIRE_DATE,GETDATE())as ishlagan_yilari
from Employees
